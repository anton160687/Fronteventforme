...
const CardImage = ({
  ...
  return (
    <div className="card-img-top card-img-hover d-flex">
      ...
      {href ? (
        <Link href={href} className="img-overlay"></Link>
      ) : (
      ...
      {badges && (
        <div className="position-absolute start-0 top-0 pt-3 ps-3">
          {badges.map((badge, indx) => {
            return (
              <span key={indx} className={`d-table badge bg-${badge[0]} mb-1`}>
                {badge[1]}
              </span>
            );
          })}
          ...
          {wishlistButton.tooltip ? (
            ...
            >
              ...
              >
                <i
                  className={
                    wishlistButton.active ? 'fi-heart-filled' : 'fi-heart'
                  }
                ></i>
              </button>
            </OverlayTrigger>
