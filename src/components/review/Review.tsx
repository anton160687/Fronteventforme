    ...
    <div className='border-bottom mb-4 pb-4'>
      <div className='d-flex justify-content-between mb-3'>
        <div className='d-flex align-items-center pe-2'>
          {avatar &&
            <div className='flex-shrink-0 bg-repeat-0 bg-position-center bg-size-cover rouded me-3'
              style={{ width: 48, height: 48, backgroundImage: 'url(' + avatar + ')' }}>
            </div>
          }
          <div>
      ...
      <div className='d-flex align-items-center'>
        <button type='button' className={`btn-like${likeActive ? ' active' : ''} ${likeClick ? '' : ' pe-none'}`} onClick={likeClick}>
          <i className='fi-like'></i>
          <span>{likeCount ? `(${likeCount})` : '(0)'}</span>
        </button>
        <div className={`border-end me-1 border-light`}>&nbsp;</div>
        <button type='button' className={`btn-dislike${dislikeActive ? ' active' : ''} ${dislikeClick ? '' : ' pe-none'}`} onClick={dislikeClick}>
